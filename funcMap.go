...
var funcMap = template.FuncMap{
	...
	//"relURL":      funcRelURL,
	"site": funcSite,
	//"knowledgeBase":     funcKnowledgeBase,
	...
	"knowledgeBaseURL":  funcKnowledgeBaseURL,
	"firstURI":          funcFirstURI,
	"lastURI":           funcLastURI,
	"generateStringID":  FuncGenerateStringID,
	"treeKnowledgeBase": funcTreeKnowledgeBase,
...
func funcSite() (Site, error) {
	finder := zorm.NewSelectFinder(tableSiteName).Append(" WHERE id=?", "minrag_site")
	site := Site{}
...
func funcSelectList(urlPathParam string, q string, pageNo int, pageSize int, sql string, values ...interface{}) (ResponseData, error) {
	...
	case tableSiteName:
		...
		zorm.Query(ctx, finder, &data, page)
		responseData.Data = data
	case tableKnowledgeBaseName:
		page.PageSize = 100
		data := make([]KnowledgeBase, 0)
		zorm.Query(ctx, finder, &data, page)
		responseData.Data = data
	case tableDocumentName:
...
// funcTreeKnowledgeBase 知识库的树形结构
func funcTreeKnowledgeBase(sql string, values ...interface{}) []*KnowledgeBase {
	ctx := context.Background()
	...
	treeKnowledgeBase := sliceKnowledgeBase2Tree(categories)

	return treeKnowledgeBase
}
...
// funcKnowledgeBaseURL 根据documentID获取所属菜单的URL,例如 /a/b/c 返回 a/b
func funcKnowledgeBaseURL(documentID string) string {
	str := strings.Trim(documentID, "/")
...
// sliceKnowledgeBase2知识库 知识库数组转树形结构
func sliceKnowledgeBase2Tree(categories []*KnowledgeBase) []*KnowledgeBase {
	categoriesMap := make(map[string]*KnowledgeBase, len(categories))
	for _, v := range categories {
		categoriesMap[v.Id] = v
	}
	treeKnowledgeBase := make([]*KnowledgeBase, 0)
	for i := 0; i < len(categories); i++ {
		knowledgeBase := categories[i]
		parent, has := categoriesMap[knowledgeBase.Pid]
		if has {
			if parent.Leaf == nil {
				parent.Leaf = make([]*KnowledgeBase, 0)
			}
			parent.Leaf = append(parent.Leaf, knowledgeBase)
		} else {
			treeKnowledgeBase = append(treeKnowledgeBase, knowledgeBase)
		}
	}
