...
var minragLocate = "zh-CN"

// initLocale 要在config初始化之后,需要获取config中的语言配置
func initLocale() {
	minragLocate = config.Locale
	defaultErr := errors.New(minragLocate + ".json " + funcT("Load failed, using default") + " zh-CN.json")
	// 打开文件
	jsonFile, err := os.Open(datadir + "/locales/" + minragLocate + ".json")
	if err != nil {
