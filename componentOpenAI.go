...
// baseComponentMap 基础组件的Map,从数据查询拼装参数
var baseComponentMap = make(map[string]IComponent, 0)
...
// initBaseComponentMap 初始化baseComponentMap
func initBaseComponentMap() {
	...
	for i := 0; i < len(cs); i++ {
		...
		if c.Parameter == "" {
			...
			baseComponentMap[c.Id] = component
			continue
		}
